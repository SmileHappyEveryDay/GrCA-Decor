...
class VDN:
    ...
    def counterfactual_advantage(self, q_values, actions):
        """
        修正维度后的反事实优势计算
        输入:
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        if self.args.flag_use_counterfact_adv:
            # ########################
            cf_adv = self.counterfactual_advantage(q_evals, u)
            cf_adv = cf_adv.squeeze(3)
            # ########################
            ...
            q_total_eval = self.eval_vdn_net(weighted_q)
        else:
            q_evals = torch.gather(q_evals, dim=3, index=u).squeeze(3)
